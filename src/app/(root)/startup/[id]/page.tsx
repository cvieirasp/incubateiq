    ...
    <>
      ...
      <section className="section_container">
        ...
        <div className="space-y-5 mt-10 max-w-4xl mx-auto">
          ...
          <h3 className="text-30-bold">Detalhes da Apresentação</h3>

          {parsedContent ? (
            <article
              dangerouslySetInnerHTML={{ __html: parsedContent }}
              className="prose max-w-4xl font-roboto break-all"
            />
          ) : (
            <p className="no-result">Nenhum detalhe informado</p>
          )}
        </div>

        <hr className="divider" />

        {editorPosts && editorPosts.select?.length > 0 && (
